...
public class B8Star : MonoBehaviour
{
    public Image star1;
    public Image star2;
    public Image star3;
    ...
    void Start()
    {
        starsEarned = PlayerPrefs.GetInt("starsEarnedB8", 0);
        highestStarsEarned = PlayerPrefs.GetInt("highestStarsEarnedB8", 0);

        if (starsEarned > highestStarsEarned)
        {
            highestStarsEarned = starsEarned;
            PlayerPrefs.SetInt("highestStarsEarnedB8", highestStarsEarned);
            PlayerPrefs.Save();
        }
        else
        {

...
public class TimerB5 : MonoBehaviour
{
    ...
    {
        if (collision.gameObject.CompareTag("B5ColStar"))
        {
            collisionIndicatorText.gameObject.SetActive(true);
            collisionIndicatorText.text = "Collision Detected!"; // show collision indicator text
            PlayerPrefs.SetFloat("capturedTime", currentTime);
            PlayerPrefs.SetInt("starsEarnedB5", starsEarned);
    ...
    private IEnumerator LoadNextScene()
    {
        ...
        SceneManager.LoadScene("CorrectB5");
    }
}
